...
jmethodID terminateMessageID;

extern jint JNICALL naMain(JNIEnv *pEnv, jobject pObj, jobjectArray pArgv, jobject pMainActivity);

extern void onAudioCaptureStateChange(int i);extern void onAudioPlaybackStateChange(int i);
...
void JNICALL naOnAppStateChange(JNIEnv *pEnv, jobject pObj, jint state)
{
    ...
    switch(state) {
        ...
        case 2:
            ...
            break;
        case 3:
            onAudioCaptureStateChange(3);onAudioPlaybackStateChange(3);
            break;
        case 4:
            onAudioCaptureStateChange(4);onAudioPlaybackStateChange(4);
            break;
        case 5:
        ...
        pMainActivity)
{
    ...
    terminateMessageID = (*pEnv)->GetMethodID(pEnv, cachedMainActivityCls,
            "terminateApp", "()V"); 
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* pVm, void *reserved)
{
    ...
    nm[0].name = "naMain";
    nm[0].signature =
            "([Ljava/lang/String;Lcom/example/sending_app1/sending_app1;)I";
    ...
    jclass cls = (*env)->FindClass(env,
            "com/example/sending_app1/sending_app1");
...
{
    if (terminateMessageID != NULL) {
        JNIEnv *pEnv;
        (*cachedJvm)->AttachCurrentThread(cachedJvm, &pEnv, NULL);
        (*pEnv)->CallVoidMethod(pEnv, cachedActivityObj, terminateMessageID);
    }
}
